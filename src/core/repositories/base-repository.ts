...
export abstract class BaseRepository<T extends BaseModel> {
  ...
  public async getAll(filters: PaginationFilters, whereOptions?: FindOptionsWhere<T>, relations?: string[]) {
    ...
    const data = await this.repo.find({ take: take, skip: skip, where: whereOptions, relations: relations});
  ...
  public async getOneById(id: number, relations: string[] | FindOptionsRelations<T> | undefined = undefined) {
    return await this.repo.findOne({where:{ id: id } as FindOptionsWhere<T>, relations: relations});
  }
